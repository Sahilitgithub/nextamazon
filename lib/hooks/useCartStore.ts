...
export const useCartService = () => {
    ...
    return { 
        ...
        decrease: (item: OrderItem) => {
            ...
            if(!exist) return 
            const updateCartItems = exist.qty === 1 ? items.filter((x) => x.slug !== item.slug) : items.map((x) => item.slug ? {...exist, qty: exist.qty - 1}: x) 
            const {itemsPrice, shippingPrice, taxPrice, totalPrice} = calcPrice(updateCartItems);
            cartStore.setState({
                items: updateCartItems,
                itemsPrice,
...
const calcPrice = (items: OrderItem[]) => {
    const itemsPrice = round2(
        items.reduce((a, item) => a + item.price * item.qty, 0)
    ),
